...
export interface MemoryMetadata {
    // Tweet-specific data (if applicable)
    tweet?: {
        id: string;
        replyToId?: string;
        threadId?: string;
    ...
    // Performance data (if part of a performance)
    performance?: {
        id: string;
        prompt: string;
        currentBeat: string;
        ...
        actorNumber?: number;  // Only present for performance tweets
    };
    ...
    // Allow for additional custom metadata
    [key: string]: any;
}

// Performance context for state composition
...
// Memory content with performance awareness
export interface PerformanceAwareContent extends Content {
...
export interface PerformanceContent extends Content {
    metadata: {
...
export abstract class Service {
    protected static instance: Service | null = null;
...
export interface PublishToEndpoint {
    url: string;
    agentId: string;
    name: string;
    apiKey: string;
...
export interface DirectClientConfig {
    baseUrl: string;
    apiKey?: string;
    networkConfig?: AgentNetwork;
}


export interface AgentRole {
    type: 'director' | 'actor';
    order?: number; // For actors (0-3)
}

// Message type hierarchy for wrestling performance system

export type MessageSource =
    | 'director'   // Control messages from director
    | 'actor'      // Performance messages from actors
    | 'system';    // Internal state/sync messages

// Metadata types to keep message payloads clear
...
export interface PerformanceEvent {
    type: 'safety' | 'error' | 'reaction' | 'officiating' | 'ending';
    timestamp: number;
    actorNumber: number;
    description: string;
}
...
export interface BasePerformanceMessage {
    id: string;
    timestamp: number;
...
export interface ControlMessage extends BasePerformanceMessage {
    id: string;
    timestamp: number;
    source: 'director';
    ...
    payload: {
        text: string;
        performanceId: string;
        state?: Partial<PerformanceMetadata>;
    };
}

export interface ActorMessage extends BasePerformanceMessage {
    id: string;
    timestamp: number;
    source: 'actor';
    action: PerformanceActionType;
...
export interface SyncMessage extends BasePerformanceMessage {
    id: string;
    timestamp: number;
    source: 'system';
    action: PerformanceActionType;
    ...
    payload: {
        ...
        requesterId?: string;
        text: string;
        state?: PerformanceMetadata;
    };
}

// Union type for all performance messages
export type PerformanceNetworkMessage =
    | ControlMessage
    | ActorMessage
    | SyncMessage;

// Message handlers
export interface PerformanceMessageHandler {
    handleMessage(message: PerformanceNetworkMessage): Promise<void>;
}

export interface DirectorMessageHandler extends PerformanceMessageHandler {
    handleActorMessage(message: ActorMessage): Promise<void>;
    handleSyncRequest(message: SyncMessage): Promise<void>;
    broadcastBeat(beat: string): Promise<void>;
    broadcastEnd(reason: PerformanceMetadata['endReason']): Promise<void>;
    notifyRecovery(): Promise<void>;
}

export interface ActorMessageHandler extends PerformanceMessageHandler {
    handleControlMessage(message: ControlMessage): Promise<void>;
    handleSyncResponse(message: SyncMessage): Promise<void>;
    broadcastAction(text: string): Promise<void>;
...
export interface MessageBroadcaster {
    broadcast(message: PerformanceNetworkMessage): Promise<void>;
}

export interface IPerformanceService extends Service {
    initialize(runtime: IAgentRuntime): void;
    manager: {
        getCurrentState(): Promise<PerformanceMetadata | null>;
        generateNewBeat(): Promise<string>;
        updateState(state: Partial<PerformanceMetadata>): Promise<void>;
        syncState(newState: PerformanceMetadata): Promise<void>;
        initializePerformance(prompt: string): Promise<void>;
        endPerformance(reason: string): Promise<void>;
        generateNewBeat(): Promise<string>;
        validateState(state: PerformanceMetadata): boolean;
        findExistingLivePerformance(): Promise<PerformanceMetadata | null>;
        setInitialized(): void;
        saveStateToMemory(state: PerformanceMetadata): Promise<void>;
        composeGenerationState(): Promise<State>;
        getLastSpeaker(): Promise<{actorNumber: number, timestamp: number} | null>;
        canActorSpeak(actorNumber: number): Promise<boolean>;
        queueMessage(text: string, speakerId: UUID, actorNumber?: number): Promise<void>;
        addMessage(text: string, speakerId: UUID, actorNumber?: number): Promise<void>;
        isStale(): boolean;
        deleteAllMemories(): Promise<void>;
        deleteAllPerformanceData(): Promise<void>;
        generateInitialPrompt(): Promise<string>;
        isInInitialization(): boolean;
    };
    messageHandler: DirectorMessageHandler | ActorMessageHandler;
